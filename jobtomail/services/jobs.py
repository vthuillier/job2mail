...
def create_job(kind: str, params: dict[str, Any] | None = None) -> str:
    job_id = uuid.uuid4().hex
    db.create_job_row(job_id, kind, params)
    return job_id


def submit_job(job_id: str, fn: Callable[[], dict[str, Any]], *, kind: str) -> None:
    def _run() -> None:
        db.update_job_row(job_id, status="running")
        try:
            result = fn()
        except Exception as e:
            logger.exception("Job %s (%s) échoué", job_id, kind)
            db.update_job_row(job_id, status="error", error=str(e))
        else:
            db.update_job_row(job_id, status="done", result=result)
